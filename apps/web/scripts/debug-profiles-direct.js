...
async function debugProfilesDirect() {
    ...
    try {
        ...
        if (structureError) {
            console.log(
                "ℹ️ [DEBUG-PROFILES] No se puede obtener estructura, continuando...",
            );
        } else {
        ...
        if (countError) {
            console.log(
                "❌ [DEBUG-PROFILES] Error contando registros:",
                countError.message,
            );
        } else {
        ...
        if (allError) {
            ...
            allProfiles.forEach((profile, index) => {
                ...
                console.log(
                    `     Email: "${profile.email}" (tipo: ${typeof profile.email})`,
                );
        ...
        if (exactError) {
            console.log(
                "❌ [DEBUG-PROFILES] Error en búsqueda exacta:",
                exactError.message,
            );
        } else {
            console.log(
                "✅ [DEBUG-PROFILES] Búsqueda exacta:",
                exactMatch?.length || 0,
                "coincidencias",
            );
        ...
        if (ilikeError) {
            console.log(
                "❌ [DEBUG-PROFILES] Error en búsqueda ILIKE:",
                ilikeError.message,
            );
        } else {
            console.log(
                "✅ [DEBUG-PROFILES] Búsqueda ILIKE:",
                ilikeMatch?.length || 0,
                "coincidencias",
            );
        ...
        if (allProfiles) {
            ...
            console.log(
                "✅ [DEBUG-PROFILES] Búsqueda manual:",
        ...
        try {
            ...
            if (constraintsError) {
                console.log(
                    "ℹ️ [DEBUG-PROFILES] No se pueden obtener constraints, continuando...",
                );
            } else {
            ...
            console.log(
                "ℹ️ [DEBUG-PROFILES] Función get_table_constraints no disponible",
            );
        }
        ...
        try {
            const { data: rawQuery, error: rawError } = await supabase.rpc("exec_sql", {
                sql_query: `
            SELECT 
              id, 
              email, 
          ...
          `,
            ...
            if (rawError) {
                console.log(
                    "ℹ️ [DEBUG-PROFILES] Función exec_sql no disponible, continuando...",
                );
            } else {
        ...
        try {
            const { data: transactions, error: transError } = await supabase.rpc(
                "get_active_transactions",
            );

            if (transError) {
                console.log(
                    "ℹ️ [DEBUG-PROFILES] No se pueden obtener transacciones, continuando...",
                );
            } else {
            ...
            console.log(
                "ℹ️ [DEBUG-PROFILES] Función get_active_transactions no disponible",
            );
        }
