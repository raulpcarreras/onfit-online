...
async function debugUsers() {
  try {
    ...
    authUsers.users.forEach((user, i) => {
      log('yellow', `   ${i + 1}. ${user.email} (ID: ${user.id.substring(0, 8)}...)`);
      log('cyan', `      - is_super_admin: ${user.app_metadata?.is_super_admin || false}`);
      log('cyan', `      - created: ${new Date(user.created_at).toLocaleDateString('es-ES')}`);
    });
    ...
    profiles.forEach((profile, i) => {
      log('yellow', `   ${i + 1}. ${profile.email} (ID: ${profile.id.substring(0, 8)}...)`);
      log('cyan', `      - full_name: ${profile.full_name || 'Sin nombre'}`);
      log('cyan', `      - role: ${profile.role}`);
      log('cyan', `      - created: ${new Date(profile.created_at).toLocaleDateString('es-ES')}`);
    });
    ...
    const onlyInProfiles = profileEmails.filter(email => !authEmails.includes(email));
    const onlyInAuth = authEmails.filter(email => !profileEmails.includes(email));
    
    if (onlyInProfiles.length > 0) {
      log('red', `❌ Solo en profiles (NO en auth.users): ${onlyInProfiles.join(', ')}`);
    }
    
    if (onlyInAuth.length > 0) {
      log('red', `❌ Solo en auth.users (NO en profiles): ${onlyInAuth.join(', ')}`);
    }
