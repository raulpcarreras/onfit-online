...
async function debugAuthUsersDirect() {
    console.log(
        "🔍 [DEBUG-AUTH-USERS-DIRECT] Iniciando debug directo de auth.users...\n",
    );
    ...
    try {
        ...
        if (allError) {
            console.log(
                "❌ [DEBUG-AUTH-USERS-DIRECT] Error listando usuarios:",
                allError.message,
            );
            return;
        }
        ...
        console.log(
            "✅ [DEBUG-AUTH-USERS-DIRECT] Usuarios encontrados:",
        ...
        allUsers.users.forEach((user, index) => {
            ...
            console.log(
                `     Email confirmado: ${user.email_confirmed_at ? "✅" : "❌"}`,
            );
        ...
        console.log(
            `🔍 [DEBUG-AUTH-USERS-DIRECT] 2. Búsqueda específica de: "${problemEmail}"`,
        );

        const matchingUsers = allUsers.users.filter(
            (user) => user.email === problemEmail,
        );

        if (matchingUsers.length > 0) {
            console.log(
                "✅ [DEBUG-AUTH-USERS-DIRECT] Usuarios encontrados con ese email:",
            ...
            console.log(
                "❌ [DEBUG-AUTH-USERS-DIRECT] NO se encontró ningún usuario con ese email",
            );
        }
        ...
        const similarEmails = allUsers.users.filter(
            (user) =>
                user.email &&
        ...
        if (similarEmails.length > 0) {
            console.log(
                "✅ [DEBUG-AUTH-USERS-DIRECT] Emails similares encontrados:",
        ...
        if (strangeEmails.length > 0) {
            console.log(
                "⚠️ [DEBUG-AUTH-USERS-DIRECT] Emails extraños encontrados:",
                strangeEmails.length,
            );
        ...
        if (duplicateEmails.length > 0) {
            console.log(
                "⚠️ [DEBUG-AUTH-USERS-DIRECT] Emails duplicados encontrados:",
        ...
        const specialCharEmails = allUsers.users.filter(
            (user) =>
                user.email &&
        ...
        if (specialCharEmails.length > 0) {
            console.log(
                "✅ [DEBUG-AUTH-USERS-DIRECT] Emails con caracteres especiales:",
                specialCharEmails.length,
            );
            ...
            console.log(
                "ℹ️ [DEBUG-AUTH-USERS-DIRECT] No hay emails con caracteres especiales",
            );
        }

        // 7. Verificar si hay usuarios con emails que contengan números
        console.log("\n🔍 [DEBUG-AUTH-USERS-DIRECT] 7. Emails que contienen números:");
        const numericEmails = allUsers.users.filter(
            (user) => user.email && /\d/.test(user.email),
        );

        if (numericEmails.length > 0) {
            console.log(
                "✅ [DEBUG-AUTH-USERS-DIRECT] Emails con números:",
                numericEmails.length,
            );
            numericEmails.forEach((user) => {
                ...
                console.log(
                    "    Números encontrados:",
                    user.email.match(/\d/g)?.join("") || "Ninguno",
                );
            });
        } else {
