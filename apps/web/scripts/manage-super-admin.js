...
async function toggleSuperAdmin(email, enable) {
  try {
    ...
    log('blue', `${enable ? '🔼' : '🔽'} ${enable ? 'Otorgando' : 'Quitando'} super admin...`);
  ...
  try {
    ...
    if (superAdmins.length === 0) {
      ...
      superAdmins.forEach((user, i) => {
        log('cyan', `   ${i + 1}. ${user.email} (ID: ${user.id.substring(0, 8)}...)`);
        log('yellow', `      Creado: ${new Date(user.created_at).toLocaleString('es-ES')}`);
        log('yellow', `      Último login: ${user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleString('es-ES') : 'Nunca'}`);
      });
    }
...
async function verifyUserPermissions(email) {
  try {
    ...
    if (accessError) {
      ...
      log('green', `✅ Puede acceder a otros perfiles: ${otherProfiles.length > 0 ? 'Sí' : 'No hay otros usuarios'}`);
    }
