...
async function debugDatabaseStructure() {
  console.log('🔍 [DEBUG-DB-STRUCTURE] Iniciando debug de estructura de base de datos...\n');
  ...
  try {
    ...
    try {
      ...
      if (constraintsError) {
        console.log('ℹ️ [DEBUG-DB-STRUCTURE] Función get_table_constraints no disponible');
        ...
        if (altError) {
          console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo constraints:', altError.message);
        } else {
          console.log('✅ [DEBUG-DB-STRUCTURE] Constraints encontrados:', altConstraints.length);
    ...
    try {
      ...
      if (indexesError) {
        ...
        if (altIndexesError) {
          console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo índices:', altIndexesError.message);
        } else {
          console.log('✅ [DEBUG-DB-STRUCTURE] Índices encontrados:', altIndexes.length);
          altIndexes.forEach(index => {
            console.log(`  - ${index.index_name || 'Sin nombre'}: ${index.index_type || 'Tipo desconocido'}`);
          });
        }
      } else {
    ...
    try {
      ...
      if (triggersError) {
        console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo triggers:', triggersError.message);
      } else {
        console.log('✅ [DEBUG-DB-STRUCTURE] Triggers encontrados:', triggers.length);
      ...
      console.log('ℹ️ [DEBUG-DB-STRUCTURE] Error accediendo a triggers:', e.message);
    }
    ...
    try {
      ...
      if (functionsError) {
        console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo funciones:', functionsError.message);
      } else {
        console.log('✅ [DEBUG-DB-STRUCTURE] Funciones relacionadas encontradas:', functions.length);
      ...
      console.log('ℹ️ [DEBUG-DB-STRUCTURE] Error accediendo a funciones:', e.message);
    }
    ...
    try {
      ...
      if (policiesError) {
        ...
        if (altPoliciesError) {
          console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo políticas:', altPoliciesError.message);
        } else {
          console.log('✅ [DEBUG-DB-STRUCTURE] Políticas encontradas:', altPolicies.length);
          if (altPolicies.length === 0) {
            console.log('  - No hay políticas RLS configuradas');
          } else {
            altPolicies.forEach(policy => {
              console.log(`  - ${policy.policy_name}: ${policy.policy_definition || 'Sin definición'}`);
            });
          }
        }
      } else {
        console.log('✅ [DEBUG-DB-STRUCTURE] Políticas encontradas:', policies.length);
        if (policies.length === 0) {
          console.log('  - No hay políticas RLS configuradas');
        } else {
          policies.forEach(policy => {
            console.log(`  - ${policy.policyname}: ${policy.cmd} ${policy.qual || 'Sin condición'}`);
          });
        }
      }
    } catch (e) {
      console.log('ℹ️ [DEBUG-DB-STRUCTURE] Error accediendo a políticas:', e.message);
    }
    ...
    try {
      ...
      if (uniqueError) {
        console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo constraints únicos:', uniqueError.message);
      } else {
        console.log('✅ [DEBUG-DB-STRUCTURE] Constraints únicos encontrados:', uniqueConstraints.length);
        uniqueConstraints.forEach(constraint => {
          console.log(`  - ${constraint.table_name}.${constraint.constraint_name}: ${constraint.constraint_type}`);
        });
      }
    } catch (e) {
      console.log('ℹ️ [DEBUG-DB-STRUCTURE] Error accediendo a constraints únicos:', e.message);
    }
    ...
    try {
      const { data: validationFunctions, error: validationError } = await supabase
        ...
        .select('*')
        .eq('routine_schema', 'public')
        .or('routine_name.ilike.%validate%,routine_name.ilike.%check%,routine_name.ilike.%constraint%');
      
      if (validationError) {
        console.log('❌ [DEBUG-DB-STRUCTURE] Error obteniendo funciones de validación:', validationError.message);
      } else {
        console.log('✅ [DEBUG-DB-STRUCTURE] Funciones de validación encontradas:', validationFunctions.length);
      ...
      console.log('ℹ️ [DEBUG-DB-STRUCTURE] Error accediendo a funciones de validación:', e.message);
    }
