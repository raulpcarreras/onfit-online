...
async function debugAuthUsers() {
    ...
    try {
        ...
        if (allError) {
            console.log(
                "❌ [DEBUG-AUTH-USERS] Error listando usuarios:",
                allError.message,
            );
            return;
        }
        ...
        if (matchingUsers.length > 0) {
            console.log(
                "✅ [DEBUG-AUTH-USERS] Usuarios encontrados con ese email:",
            ...
            console.log(
                "❌ [DEBUG-AUTH-USERS] NO se encontró ningún usuario con ese email",
            );
        }
        ...
        if (similarEmails.length > 0) {
            console.log(
                "✅ [DEBUG-AUTH-USERS] Emails similares encontrados:",
        ...
        if (profilesError) {
            console.log(
                "❌ [DEBUG-AUTH-USERS] Error obteniendo perfiles:",
                profilesError.message,
            );
        } else {
            const profileIds = new Set(allProfiles.map((p) => p.id));
            const usersWithoutProfile = allUsers.users.filter(
                (user) => !profileIds.has(user.id),
            );

            if (usersWithoutProfile.length > 0) {
                console.log(
                    "⚠️ [DEBUG-AUTH-USERS] Usuarios SIN perfil encontrados:",
        ...
        if (duplicateEmails.length > 0) {
            console.log(
                "⚠️ [DEBUG-AUTH-USERS] Emails duplicados encontrados:",
        ...
        if (strangeEmails.length > 0) {
            console.log(
                "⚠️ [DEBUG-AUTH-USERS] Emails extraños encontrados:",
