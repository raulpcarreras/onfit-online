...
export async function POST(req: Request) {
  try {
    ...
    const { full_name, email, password, role, is_super_admin, email_confirmed } = body as {
      full_name: string;
      email: string;
      password: string;
    ...
    console.log("🔍 [CREATE-USER] Datos extraídos:", { full_name, email, role, is_super_admin, email_confirmed });
    ...
    console.log("🔗 [CREATE-USER] URL:", process.env.NEXT_PUBLIC_SUPABASE_URL ? "✅ Configurada" : "❌ Faltante");
    console.log("🔗 [CREATE-USER] Service Role Key:", process.env.SUPABASE_SERVICE_ROLE_KEY ? "✅ Configurada" : "❌ Faltante");
    ...
    if (listError) {
      console.log("❌ [CREATE-USER] Error listando usuarios auth:", listError.message);
      return NextResponse.json({ error: listError.message }, { status: 500 });
    }
    
    console.log("✅ [CREATE-USER] Usuarios auth listados:", existingUsers.users.length);

    const emailExistsInAuth = existingUsers.users.some(u => 
    ...
    console.log("🔍 [CREATE-USER] Verificando email en profiles (VERIFICACIÓN AGRESIVA)...");
    ...
    if (exactError) {
      console.log("❌ [CREATE-USER] Error verificando email exacto:", exactError.message);
    ...
    if (ilikeError) {
      console.log("❌ [CREATE-USER] Error verificando con ILIKE:", ilikeError.message);
    ...
    if (likeError) {
      console.log("❌ [CREATE-USER] Error verificando con LIKE:", likeError.message);
    ...
    if (allError) {
      console.log("❌ [CREATE-USER] Error listando todos los perfiles:", allError.message);
    ...
    const manualMatches = allProfiles.filter(profile => {
      ...
      return (
        ...
        profileEmail.replace(/[^a-zA-Z0-9@.-]/g, "") === searchEmail.replace(/[^a-zA-Z0-9@.-]/g, "")
      );
    });
    ...
    const uniqueMatches: Match[] = allMatches.filter((match, index, self) => 
    ...
    if (uniqueMatches.length > 0) {
      ...
      uniqueMatches.forEach((match, index) => {
        console.log(`    ${index + 1}. ID: ${match.id}, Email: "${match.email}", Nombre: ${match.full_name ?? "(sin nombre)"}`);
      });
      return NextResponse.json({ error: "email_already_exists_in_profiles" }, { status: 409 });
    }
    
    console.log("✅ [CREATE-USER] Email no existe en profiles (verificación agresiva completada)");
    ...
    if (createError) {
      console.log("❌ [CREATE-USER] Error creando usuario auth:", createError.message);
    ...
    console.log("✅ [CREATE-USER] Usuario auth creado:", newUser.user.id);
            ...
            console.log("✅ [CREATE-USER] Trigger handle_new_user() creó el perfil automáticamente");
            ...
            const { data: updateData, error: updateError, count } = await adminSupabase
              .from("profiles")
              .update({ 
                full_name, 
                role, 
            ...
            if (updateError) {
              console.log("❌ [CREATE-USER] Error actualizando perfil:", updateError.message);
              ...
              if (upsertError) {
                console.log("❌ [CREATE-USER] Error en UPSERT fallback:", upsertError.message);
              ...
              const { error: upsertError2 } = await adminSupabase
                .from("profiles")
                .upsert({ 
                  id: newUser.user.id, 
                  email, 
              ...
              if (upsertError2) {
                console.log("❌ [CREATE-USER] Error en UPSERT de seguridad:", upsertError2.message);
                await adminSupabase.auth.admin.deleteUser(newUser.user.id);
                return NextResponse.json({ error: upsertError2.message }, { status: 500 });
              }
