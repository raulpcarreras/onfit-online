...
export async function POST(req: Request) {
  try {
    ...
    const { userId, email, enable } = body as { userId?: string; email?: string; enable?: boolean };
    ...
    if (!targetUserId && email) {
      const { data: list, error: listErr } = await adminSupabase.auth.admin.listUsers({ page: 1, perPage: 200 });
    ...
    const { data: updated, error: upErr } = await adminSupabase.auth.admin.updateUserById(targetUserId!, {
      app_metadata: { is_super_admin: !!enable },
    });

    if (upErr) return NextResponse.json({ error: upErr.message }, { status: 500 });

    return NextResponse.json({ ok: true, userId: targetUserId, is_super_admin: !!enable });
  } catch (error: any) {
